...
class NiceSampler(Sampler):
    ...
    def __init__(self, gen_arch, log_prob_func, log_prob_func_irr, disc_arch,
        ...
        self.energy_func_irr = lambda var: -log_prob_func_irr(var)
        ...
        self.infer_op_irr = IrrInferenceOperator(self.generator, self.energy_func_irr)
        ...
        self.z_irr, self.v_irr, self.dir_irr = self.infer_op_irr((self.z, v, dir), self.steps, self.nice_steps)
    ...
    def sample_irr(self, sess=None, num_samples=2000, nice_steps=1, num_chains=32, **kwargs):
        start = time.time()
        z, _ = sess.run([self.z_irr, self.v_irr], feed_dict={
            self.z: self.ns(num_chains), self.steps: num_samples,
            self.nice_steps: nice_steps})
        end = time.time()
        return z, end-start
