...
class NiceNetwork(object):
    ...
    def __call__(self, x, is_backward):
        x_backward, v_backward = self.backward(x)
        x_forward, v_forward = self.forward(x)
        x_final = x_backward*is_backward + x_forward*(1-is_backward)
        v_final = v_backward * is_backward + v_forward * (1 - is_backward)
...
class InferenceOperator(object):
    ...
    def __call__(self, inputs, steps, nice_steps=1):
        def nice_proposal(zv, x):
            ...
            """
            z, v = zv
            u = tf.cast(x * tf.random_uniform([]) < 0.5, tf.float32)
            z_, v_ = self.network([z, v], is_backward=u) #(tf.random_uniform([]) < 0.5))
            return z_, v_
        ...
        def fn(zv, x):
            ...
            z_, v_ = tf.scan(nice_proposal, x, (z, v), back_prop=False)
            z_, v_ = z_[-1], v_[-1]
...
class IrrInferenceOperator(object):
    def __init__(self, network, energy_fn):
        self.network = network
        self.energy_fn = energy_fn
    ...
    def __call__(self, inputs, steps, nice_steps=1):
        def nice_proposal(zvd, x):
            ...
            """
            z, v, d = zvd
            z_, v_ = self.network([z, v], is_backward=d) #(tf.random_uniform([]) < 0.5))
            d_ = d
            return z_, v_, d_
        ...
        def fn(zvd, x):
            ...
            """
            z, v, d = zvd
            v = 0.6*v + 0.8*tf.random_normal(shape=tf.stack([tf.shape(z)[0], self.network.v_dim]))
            # z_, v_ = self.network([z, v], is_backward=(tf.random_uniform([]) < 0.5))
            z_, v_, d_ = tf.scan(nice_proposal, x * tf.random_uniform([]), (z, v, d), back_prop=False)
            z_, v_, d_ = z_[-1], v_[-1], d_[-1]
            ep = hamiltonian(z, v, self.energy_fn)
            en = hamiltonian(z_, v_, self.energy_fn)
            accept = metropolis_hastings_accept(energy_prev=ep, energy_next=en)
            z_ = tf.where(accept, z_, z)
            v_ = tf.where(accept, v_, v)
            d_ = tf.where(accept, d_, 1.0-d)
            return z_, v_, d_
